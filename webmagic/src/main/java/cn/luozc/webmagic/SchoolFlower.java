...
public class SchoolFlower implements PageProcessor {
    ...
    public void process(Page page) {
        ...
        if (page.getUrl().regex(REGEX_PAGE_URL).match()) {
            ...
            //要按条件筛选
            String diggNum = page.getHtml().xpath("//div[@class='clearfix']/text()").regex("\\d+").toString();
            //点赞数要超过300
            if (Integer.parseInt(diggNum) > 1000) {
                //得到照片url
                ...
                try {
                     ...
                     * String 保存路径
                     */
                    SpiderDownload.download( "http://www.521609.com"+photoURL, nickname + ".jpg", "F:\\schoolFlowerImage\\");
